...
O procedimento adotado parte de uma frase correta, modificada de forma controlada. Cada
variante introduz um único desvio, de tipo conhecido. Todos os métodos são aplicados às
...
Seis frases auxiliares completam o corpus. Duas comparam complexidade sintática sob o mesmo
número de palavras, duas comparam comprimento de dependência com as mesmas palavras em
ordens distintas, e duas formam um par de permutação. São necessárias porque os descritores
só se distinguem quando o comprimento é mantido fixo.

Os onze métodos distribuem-se por quatro dimensões, cada uma associada a uma verificação
...
Nenhum modelo foi treinado ou ajustado. Todos os recursos são de prateleira, e os limites
observados adiante pertencem a eles, e não ao método: a substituição do analisador sintático
altera o resultado sem alterar uma linha da regra. O analisador declara acerto de 95,6\% nos
traços morfológicos e de 86,0\% nos arcos de dependência, valores que a
Seção~\ref{sec:empiricos} retoma ao discutir os falsos negativos.
